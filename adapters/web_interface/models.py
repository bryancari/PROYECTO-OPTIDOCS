...
class LabelFieldModel(models.Model):
    ...
    value_format = models.ForeignKey(
        LabelFieldValue, on_delete=models.CASCADE, verbose_name="Formato de la etiqueta"
    )
...
class ConfigDocModel(models.Model):
    ...
    ocr_chosen = models.ForeignKey(
        OCRProvider, on_delete=models.CASCADE, verbose_name="Servicio OCR"
    )
    type_doc = models.ForeignKey(
        DocumentTypeModel, on_delete=models.CASCADE, verbose_name="Tipo de documento"
    )
    language = models.ForeignKey(
        Language, on_delete=models.CASCADE, verbose_name="Lenguaje"
    )
...
class DocumentModel(models.Model):
    ...
    result = models.OneToOneField(
        ResultOCRModel, on_delete=models.CASCADE, verbose_name="Resultado del OCR"
    )
    processed = models.BooleanField(default=False, verbose_name="Estado actual")
    doc_file = models.FileField(
        upload_to="documentos/", verbose_name="Archivo", null=False
    )
    created_at = models.DateTimeField(
